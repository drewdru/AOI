...
def canny(colorModelTag, currentImageChannelIndex, pixels, imgSize,
    ...
    for x in range(1, width-1):
        ...
        for y in range(1, height-1):
            ...
            pxB = (sobel_x[0][0] * G[x-1,y-1][2]) + (sobel_x[0][1] * G[x,y-1][2]) + \
                (sobel_x[0][2] * G[x+1,y-1][2]) + (sobel_x[1][0] * G[x-1,y][2]) + \
                (sobel_x[1][1] * G[x,y][2]) + (sobel_x[1][2] * G[x+1,y][2]) + \
                (sobel_x[2][0] * G[x-1,y+1][2]) + (sobel_x[2][1] * G[x,y+1][2]) + \
                (sobel_x[2][2] * G[x+1,y+1][2])
